...
class Menus : NSObject {
    ...
    class func loadMenuForDay(day : NSInteger, month : NSInteger, year : NSInteger, offset : NSInteger) -> NSData? {
        ...
        if fileExists { //if cached, return cached file
            var error : NSError?
            var cached = NSData(contentsOfFile: path, options: nil, error: &error)
        ...
        if let menuItems = menuArray {
            for item in menuItems {
                ...
                if !contains(ignoreList, item_name.stringValue()) {
                    ...
                    var attributeMatches = regex.matchesInString(item_name.stringValue(), options: nil, range: NSMakeRange(0, (item_name.stringValue() as NSString).length)) as NSArray
